...
def CalculateFitness(DecryptionKey, EncryptedText):
    global IS_OPTIMAL
    global FitnessCallsCoutner

    IS_OPTIMAL = False
    FitnessCallsCoutner += 1
...
def crossover(parent1, parent2):
    ...
    for i in range(index + 1, len(keys)): #fill the othwer with the letter in perant 2
...
def FixLocalMax(EncryptedText,BestDecryption, BestDecryptionFitness):
    for i in range(5):
        ...
        return BestDecryption if IS_OPTIMAL else None
...
if __name__ == '__main__':
    ...
    if counter == LOCAL_MAXIMUM and not IS_OPTIMAL:
    ...
    print("fitness calls: ", FitnessCallsCoutner)